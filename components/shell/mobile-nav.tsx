 ...
 *  the persistent Sidebar takes over. Mirrors the Sidebar's nav model. */
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        className="grid h-9 w-9 place-items-center rounded-sm text-text-secondary transition-colors hover:bg-white/[0.05] hover:text-text md:hidden"
      >
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden" role="dialog" aria-modal="true" aria-label="Navigation">
          <button aria-label="Close navigation menu" onClick={() => setOpen(false)} className="absolute inset-0 bg-black/60" />
          <div className="absolute left-0 top-0 flex h-full w-[280px] max-w-[85vw] flex-col border-r border-border bg-surface-2 shadow-e2">
